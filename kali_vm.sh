#!/bin/bash
########################################################################################
sudo mount /dev/nvme0n1p9 /home/$LOGNAME/D
...
sudo systemd-run --slice=kali_qemu.slice  --unit=steven_qemu --property="AllowedCPUs=0-15" qemu-system-x86_64 \
  ...
  --object memory-backend-file,id=mem0,size=16384M,mem-path=/dev/hugepages,prealloc=on,share=on \
  ...
  --blockdev file,node-name=f0,filename=/home/$LOGNAME/D/vm/kali-linux-2023.1-qemu-amd64.qcow2 \
  --blockdev qcow2,node-name=q0,file=f0 \
  --device virtio-blk-pci,drive=q0,iothread=io0 \
  --object iothread,id=io1 \
  --blockdev file,node-name=f1,filename=/home/$LOGNAME/L/drive.qcow2 \
